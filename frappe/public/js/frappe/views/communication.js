...
frappe.views.CommunicationComposer = class {
	...
	make() {
		...
		//! Newly Add
		//! Add a custom action button footer of the email compose box
		//! Like Send and Discard Button
		//! A pre-build method is build already in, apps/frappe/frappe/public/js/frappe/ui/dialog.js
		//! Just call the method, to show in the footer on the compose box
		//! By clicking this button, Rewrite will done
		//! WE pass sent_or_recived's value when click in reply button
		//! sent_or_received will indicate that, for what we write the email, sent a mail or reply to received mail
		//! if the value of sent_or_recieved is "Received", that's mean we make a reply forreceived mail
		//! it will show "Generate Reply" button, in footer
		//! Otherwise it will show "Professional Rewrite" button in the footer
		if (this.sent_or_received === "Received") {
			this.dialog.add_custom_action("Generate Reply", () => {
			//? Call the Function for Generate Reply
			this.generate_reply_mail()
			}, "custom-button");
		} else {
			this.dialog.add_custom_action("Professional Rewrite", () => {
			//? Call the Function for Rewrite
			this.new_test()
			}, "custom-button");
		}
	...
	get_fields() {
		...
		const fields = [
			...
			},
			{
				...
				fieldname: "tone",
				options: "Human Like\nMore Persuasive\nMore Informative\nMore Descriptive\nMoreHumorous\nUrgent\nMore Emphatic\nMore Concise",
				placeholder: __("Select a Tone For Rewrite"),
				// Other attributes for the field
			},
			// {
			// 	label: __("Professional Rewrite"),
			// 	fieldtype: "Button",
			// 	fieldname: "click",
			// 	click: () => {
			// 	//! After Click The Button, This function will Trigger, freshly made
			// 	this.new_test()
			// 	}
			// },
		];
	...
	new_test() {
		// const fields = this.dialog.fields_dict; // fetch value
		// //fields.content.set_value("hello guys"); // set value
		// let h = fields.content.get_value(); // get value
		// if(!h == ""){
		// frappe.msgprint("No content in message box");
		// } else {
		// frappe.msgprint(h);
		// } 
		const fields = this.dialog.fields_dict;
		let h = fields.content.get_value();
		let g = fields.tone.get_value();
		frappe.call({
			method: "frappe.core.doctype.communication.rewrite.rewrite_ai",
			freeze: true,
			freeze_message: __("Rewrite On Process"),
			args: {
			//! pass the content field's
			content: h, //? Here content is the name of the parameter of the method
			tone: g
			},
		callback: function (response) {
		//! This is return value, from : frappe.core.doctype.communication.email.test
		const returnedValue = response.message;
		//! Set or replace the content field value
		fields.content.set_value(returnedValue);
		}
		});
	...
	generate_reply_mail(){
		const fields = this.dialog.fields_dict; //! fetch the fields
		let tone_d = fields.tone.get_value(); //! store the tone in a variable
		frappe.call({
		method: "frappe.core.doctype.communication.rewrite.generate_reply",
		freeze: true,
		freeze_message: __("Generate Reply On Process"),
		args: {
			//! pass the message of received mail
			content: this.content, //? Here content is the name of the parameter of the method, and this.content is received mail's message
			tone: tone_d //? Here tone is the name of the parameter of the method
			},
		callback: function (response) {
		//! This is return value, from:frappe.core.doctype.communication.email.generate_reply
		const returnedValue = response.message;
		//! Set or replace the content field value
		fields.content.set_value(returnedValue);
				}
